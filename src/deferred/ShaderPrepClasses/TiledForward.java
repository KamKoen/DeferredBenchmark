...
import deferred.Main.Shader;


public class TiledForward extends PerPixelForward{
	
	
	ShaderProgram DPprogram;
	
	public TiledForward(float C, float D, int tileWidth, int tileHeight)
	{
		ComputeTile = new ShaderProgram();
        ComputeTile.attachComputeShader(folderOfAlgorithms+"ComputeTile.comp");
        ComputeTile.link();
        ComputeTile.bind();
